    ...
    (item) => {
      ...
      setTimeout(() => {
        ...
        if (item === "all") {
          ...
          setFilterWork(
            works.filter((work) => {
              const loweredTags = work.tags.map((tag) => tag.toLowerCase());
              return loweredTags.includes(item.toLowerCase());
            })
          );
        }
      }, 500);
  ...
  return (
    <>
      ...
      <div className="app__work-filter">
        {["mern", "backend", "frontend", "reactjs", "nodejs", "all"].map(
          (item) => (
            <div
              key={item}
      ...
      >
        {filterWork.map((item, index) => (
          <div className="app__work-item app__flex" key={item + index}>
